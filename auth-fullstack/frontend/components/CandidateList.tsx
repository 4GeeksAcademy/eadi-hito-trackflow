...
export default function CandidateList() {
  ...
  async function reloadRecords() {
    ...
    try {
      ...
      setError(fetchError instanceof Error ? fetchError.message : 'No se pudieron cargar las candidaturas.');
    } finally {
      setLoading(false);
    }
  }
  ...
  return (
    <main className="mx-auto max-w-7xl space-y-8 px-4 py-8 sm:px-6 lg:px-8">
      ...
      {loading ? (
        ...
        <div className="rounded-2xl border border-red-200 bg-red-50 px-4 py-3 text-red-700">{error} <button type="button" onClick={() => void reloadRecords()} className="ml-3 font-semibold underline">Reintentar</button></div>
